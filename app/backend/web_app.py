...
def google_auth():
    ...
    logger.debug(f"INTO GOOGLE AUTH")

    if not env_vars:
        return jsonify({"error": "Server configuration error"}), 500
    
    try:
        # GmailAuthenticator(env_vars["creds_path"])
        # .authenticate_user(session['user_email'], env_vars["scopes"], env_vars["token_storage_path"])
        ...
        line_number = exec_tb.tb_lineno if exec_tb else 'unknown'
        function_name = exec_tb.tb_frame.f_code.co_name if exec_tb else 'unknown'
        logger.error(f"Failed in '{function_name}' at line '{line_number}'|Google auth initiation failed: {e}")
        return jsonify({"error": "Authentication failed"}), 500

@app.route('/auth/google/callback')
def google_callback():
    """Handle Google OAuth callback."""
    logger.debug(f"INTO GOOGLE CALLBACK")
    ...
    try:
        ...
        session['credentials'] = {
            ...
            'token_uri': credentials.token_uri, # pyright: ignore[reportAttributeAccessIssue]
        ...
        line_number = exec_tb.tb_lineno if exec_tb else 'unknown'
        function_name = exec_tb.tb_frame.f_code.co_name if exec_tb else 'unknown'
        logger.error(f"Failed in '{function_name}' at line '{line_number}'|Google callback failed: {e}")
        return jsonify({"error": "Authentication callback failed"}), 500

@app.route('/dashboard')
def dashboard():
    """User dashboard after successful authentication."""
    logger.debug(f"INTO DASHBOARD")
...
def get_gmail_profile():
    ...
    logger.debug(f"INTO GET GMAIL PROFILE")
    ...
    try:
        ...
        line_number = exec_tb.tb_lineno if exec_tb else 'unknown'
        function_name = exec_tb.tb_frame.f_code.co_name if exec_tb else 'unknown'
        logger.error(f"Failed in '{function_name}' at line '{line_number}' to get Gmail profile: {e}")
        return jsonify({"error": "Failed to retrieve profile"}), 500

@app.route('/api/gmail/messages')
def get_gmail_messages():
    """Get Gmail messages using stored credentials."""
    logger.debug(f"INTO GET GMAIL MESSAGES")
    if 'credentials' not in session:
        return jsonify({"error": "Not authenticated"}), 401
    ...
    try:
        ...
        line_number = exec_tb.tb_lineno if exec_tb else 'unknown'
        function_name = exec_tb.tb_frame.f_code.co_name if exec_tb else 'unknown'
        logger.error(f"Failed in '{function_name}' at line '{line_number}' to get Gmail messages: {e}")
        return jsonify({"error": "Failed to retrieve messages"}), 500

@app.route('/logout')
def logout():
    """Logout user and clear session."""
    logger.debug(f"INTO LOGOUT")
    session.clear()
    return redirect(url_for('index'))
